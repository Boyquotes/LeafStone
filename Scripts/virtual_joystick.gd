...
# DYNAMIC: Every time the joystick area is pressed, the joystick position is set checked the touched position.
enum JoystickMode {FIXED, DYNAMIC}

@export var joystick_mode:= JoystickMode.FIXED

# VISIBILITY_ALWAYS = Always visible.
# VISIBILITY_TOUCHSCREEN_ONLY = Visible checked touch screens only.
...
var _pressed := false :
	get:
		return _pressed # TODOConverter40 Copy here content of is_pressed 
	set(mod_value):
...
var _output := Vector2.ZERO :
	get:
		return _output # TODOConverter40 Copy here content of get_output 
	set(mod_value):
		mod_value  # TODOConverter40  Non existent set function
